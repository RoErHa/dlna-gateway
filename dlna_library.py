...
class Indexer:
    ...
    def _run(self, server, force: bool):
        ...
        try:
            ...
            # No-op if a scan is already running (it re-checks bare_albums
            # between batches, so new albums will be picked up there).
            try:
                ART_FETCHER.trigger()
            except NameError:
...
# ── Album-art fetcher (Phase B) ───────────────────────────────────
# Background worker that fills album_art cache from MusicBrainz +
# Cover Art Archive for albums still missing covers after Phase A
# sibling backfill. Negative hits are cached as source='notfound' so
