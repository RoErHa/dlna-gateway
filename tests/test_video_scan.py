...
class TestReverseGeocode(unittest.TestCase):
    ...
    def test_success(self):
        with mock.patch.object(dlna_geocode.urllib.request, "urlopen",
                               return_value=self._resp({"address": {"city": "Amsterdam"}})):
            self.assertEqual(dlna_geocode.reverse_geocode(52.37, 4.90), "Amsterdam")
...
class TestPlaceFor(unittest.TestCase):
    ...
    def test_cache_hit_skips_network(self):
        self.db.geocode_put(52.37, 4.90, "Amsterdam")
        with mock.patch.object(dlna_geocode, "reverse_geocode") as rg:
            self.assertEqual(dlna_geocode.place_for(self.db, 52.37, 4.90), "Amsterdam")
            rg.assert_not_called()

    def test_miss_fetches_and_caches(self):
        with mock.patch.object(dlna_geocode, "reverse_geocode",
                               return_value="Berlin") as rg:
            self.assertEqual(dlna_geocode.place_for(self.db, 52.52, 13.40), "Berlin")
    ...
    def test_transient_not_cached(self):
        ...
        self.assertEqual(self.db.geocode_get(1.0, 2.0), (None, False))   # not cached
...
class TestScan(unittest.TestCase):
    ...
    def test_scan_inserts_with_constructed_title(self):
        ...
        self.assertEqual(v["title"], "Amsterdam_20260614_1430.mov")
