...
# ── C7: /api/acoustid/status + /api/acoustid/enrich handlers ──────

import api_playback   # noqa: E402
...
class TestAcoustIDHandlers(unittest.TestCase):
        ...
        with self.db._pool.write() as c:
            for i in range(3):
                c.execute(
                    ...
                    "VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                    ("uuid:x", f"o{i}", f"http://t/{i}", f"T{i}", "A", "Al",
                     "0:03:00", "", "audio/flac", "", ""))

    def tearDown(self):
        os.unlink(self._path)

    def test_status_shape_and_remaining_when_enabled(self):
        fetcher = AcoustIDFetcher(self.db, api_key="test-key")
        h = _MockH()
        with patch.object(api_playback, "DB", self.db), \
             patch("dlna_library.ACOUSTID_FETCHER", fetcher):
            api_playback.acoustid_status(h, {})
        code, p = h.last
        ...
        self.assertEqual(p["remaining"], 3)   # 3 bare tracks seeded

    def test_status_disabled_without_key(self):
        fetcher = AcoustIDFetcher(self.db, api_key=None)
        h = _MockH()
        with patch.object(api_playback, "DB", self.db), \
             patch("dlna_library.ACOUSTID_FETCHER", fetcher):
            api_playback.acoustid_status(h, {})
        self.assertFalse(h.last[1]["enabled"])

    def test_enrich_503_when_disabled(self):
        fetcher = AcoustIDFetcher(self.db, api_key=None)
        fetcher.trigger = MagicMock()
        h = _MockH()
        with patch("dlna_library.ACOUSTID_FETCHER", fetcher):
            api_playback.acoustid_enrich(h, {})
        self.assertEqual(h.last[0], 503)
        fetcher.trigger.assert_not_called()

    def test_enrich_triggers_when_enabled(self):
        fetcher = AcoustIDFetcher(self.db, api_key="test-key")
        fetcher.trigger = MagicMock()
        h = _MockH()
        with patch("dlna_library.ACOUSTID_FETCHER", fetcher):
        ...
        fetcher.trigger.assert_called_once()


class TestBareMetadataCount(unittest.TestCase):

    def test_count_excludes_tracks_with_overrides(self):
        fd, path = tempfile.mkstemp(suffix=".db"); os.close(fd)
        try:
            db = LibraryDB(db_file=path)
            with db._pool.write() as c:
                for i in range(4):
                    c.execute(
                        ...
                        ("uuid:x", f"o{i}", f"http://t/{i}", f"T{i}", "A", "Al",
                         "0:03:00", "", "audio/flac", "", ""))
            self.assertEqual(db.bare_metadata_count(), 4)
            db.metadata_override_mark_notfound("http://t/0")
            db.metadata_override_set("http://t/1", "acoustid", artist="X")
            self.assertEqual(db.bare_metadata_count(), 2)
        finally:
            os.unlink(path)
