...
<!-- badges: generated by tools/badges.py -->
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![Python 3.14+](https://img.shields.io/badge/python-3.14%2B-blue.svg)](https://www.python.org/)
[![Tests](https://img.shields.io/badge/tests-192%20checks%20%C2%B7%201%2C438%20unit%20%C2%B7%20276%20browser-brightgreen.svg)](#testing)
[![Lint: ruff](https://img.shields.io/badge/lint-ruff%20%C2%B7%200%20violations-brightgreen.svg)](#code-quality-gates)
[![Quality gates](https://img.shields.io/badge/quality%20gates-5%20%C2%B7%20all%20at%20zero-brightgreen.svg)](#code-quality-gates)
...
Five gates run **inside `python tests/run_all.py --offline`** — they aren't a
...
| **T0.BADGES** — README badge numbers | the badges match what the suites actually count | `tools/badges.py --write` |
...
The debt is now paid off — **89/89 modules within target** — so it behaves as
a flat limit in practice, without ever having needed a flag day.

The badge gate exists because those numbers used to be typed by hand, and a
number nobody verifies is worse than no number — a reader believes it. The
strip is rendered from the suites by `tools/badges.py`: the **unit** and
**browser** counts are collected without running anything and checked by
`tests/test_badges.py`, while the **checks** count is verified here, from the
total this run has just finished counting. That one cannot live in a unit
test, because the only way to know how many checks the runner performs is to
perform them — and a unit test that ran the runner would recurse into itself.
It is last on purpose: it counts itself.
...
**[docs/ARCHITECTURE.PDF](docs/ARCHITECTURE.PDF)** carries two drawings
that answer deliberately different questions, plus reference tables for
every program, tool, device, external service and scheduled job.

*Page 1 — four journeys (what happens when).* Each lane reads left to
right:

| | |
|---|---|
| **A track reaches your ears** | files → index → `library.db` → choose → bytes. The renderer pulls from `:8200` itself, so the gateway is never in the audio path. |
| **The library learns what it knows** | beets → artist ids → facts → credits → housekeeping. Left-to-right is also the order you must run them in. |
| **A phone clip becomes a browsable memory** | the only journey with machine learning in it — and the ML is Immich's, not ours. |
| **Four ways to reach the same library** | PWA, Naim, CarPlay, LG TV. One index, four protocols. |

*Page 2 — what feeds what (what this is).* Three concentric layers, which
are the real dependency order rather than a drawing convention:
**content** (the files you own and the index over them — the only layer
rebuildable from disk alone), **enrichment** (what we have *learned*
about it: art, lyrics, credits, artist facts, book series, video people
— none of it regenerable, which is exactly why every one of those tables
survives a re-index), and **surfaces** (the four clients, as peers).
Seventeen outside services sit around the rings, each arrow pointing at
the layer it actually touches and coloured by target. The shape that
falls out is the point: **most of the outside world feeds enrichment,
not content — the library is yours, the facts about it are borrowed.**

Both pages are generated, not drawn by hand: after an architecture
change run `python3 tools/gen_architecture_pdf.py` (needs `reportlab`,
which is not a gateway dependency).
