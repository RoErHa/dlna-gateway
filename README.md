...
[![Tests](https://img.shields.io/badge/tests-191%20checks%20%C2%B7%201%2C158%20unit%20%C2%B7%20238%20browser-brightgreen.svg)](#testing)
...
Four gates run **inside `python tests/run_all.py --offline`** — they aren't a
separate command you can forget. Each is deliberately sized to sit at **zero
violations**, so any hit is a real regression rather than a backlog you learn
to ignore:

| Gate | Enforces | Fix when it fails |
|---|---|---|
| **T0.LINT** — [ruff](https://docs.astral.sh/ruff/) | `F, E9, W, B, RET, UP, C4, PIE, ASYNC` (`pyproject.toml`) | `ruff check . --fix` |
| **T0.SIZE** — module line ratchet | every module may shrink, never grow; 400-line target | split the module, or `tools/regen_size_budget.py` |
| `tests/test_lock_sync.py` | `requirements.lock` matches `requirements.txt` | `tools/regen_lock.py` |
| `tests/test_no_silent_swallows.py` | no `except Exception: pass` | log it, or narrow the `except` |

The rules are picked because each catches a **defect class, not a style
preference**. **ASYNC matters most here**: the whole edge is an ASGI app, so
one blocking call inside an `async def` serialises every request. It has
already caught four real ones — `os.path.isfile()` on an external USB volume
inside the video routes, where a spun-down disk stalls the event loop (and
every open SSE stream) for seconds.

The size gate is a **ratchet, not a flat limit**: a flat 400-line rule would
have failed on day one for 15 modules and been switched off within the week.
The debt is now paid off — **84/84 modules within target** — so it behaves as
a flat limit in practice, without ever having needed a flag day.

ruff is a **dev-only dependency**: if it isn't installed the gate SKIPs rather
than reddening a clean clone.

```bash
pip install -r requirements-dev.txt   # ruff, pytest, playwright
ruff check .                          # the lint gate on its own
```

> **Honest caveat:** there is no CI on this repo — it's one person's
> single-developer project, and the badges above are a snapshot regenerated by
> `python3 tools/regen_badges.py --apply`, not a live build status.
