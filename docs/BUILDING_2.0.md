...
- [~] **R2 — SSE push** — BACKEND INFRA DONE (slice 1): `dlna_events.EventBus`/`EVENTS` (thread-safe `publish()` → `loop.call_soon_threadsafe`, fan-out to per-subscriber `asyncio.Queue`, drop-on-full) + native `GET /api/events` `text/event-stream` (hello frame, 15 s comment heartbeat, unsubscribe on disconnect); loop bound in `_lifespan`. 12 unit tests; verified live (`curl -N /api/events` → `event: hello`). **Slice 2 (next):** wire real publishers — `RendererQueue` now-playing/state, `Indexer` progress, device add/leave — each a one-line `EVENTS.publish({...})`; then the PWA `EventSource('/api/events')` replacing the `/api/state` + `pollIndex` polls (Playwright tests; test-first). NB SSE is ASGI-only — not on the stdlib `run-2.0.sh` server.
